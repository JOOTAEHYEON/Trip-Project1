...
%>
    ...
    <h1 class="row  tway1" style="margin-bottom:60px;" >검색 결과</h1>
            ...
            <div class="card-body">
                <p class="card-text">
                    ...
                    <div style="font-size:16px;">${vo1.name}</div>                                    
                      <c:set var="priceWithoutComma" value="${fn:replace(vo1.price, ',', '')}" />
           		 <c:set var="priceInt" value="${priceWithoutComma ne '' ? Integer.parseInt(priceWithoutComma) : 0}" />
            
          		  <div scope="col" id="price" style="vertical-align: middle;">             
              		  <fmt:formatNumber type="currency" currencyCode="KRW" value="${priceInt}" pattern="###,###원"/>
           		 </div>
                </p>
            </div>
        </a>
   ...
   <div class=" display"  style="margin-top:180px;">
      <button onclick="onPage('first')" class="pageNumber2" style="margin-right:10px;margin-left:10px"> << </button>
      <button onclick="onPage('prev')" class=" pageNumber2" style="margin-right:10px;"> < </button>

      <c:forEach begin="1" end="${totalPages}" var="pageNumber">
       <%--   <button onclick="onPage(${pageNumber})" class= "pageNumber1" value = "${pageNumber}">${pageNumber} </button> --%>
          <button onclick="onPage(${pageNumber})" class="pageNumber1 ${pageNumber == nowPage ? 'current' : ''}" value="${pageNumber}">${pageNumber} </button>
      </c:forEach>

     <button onclick="onPage('next')" class="pageNumber2" style="margin-right:10px;margin-left:10px"> > </button>
      <button onclick="onPage('last')" class=" pageNumber2 " > >> </button>
   </div>  
</div>
...
<script>
   function onPage(type) {
      const nowPage = ${param.pageNumber}; 
       var num = 1;
