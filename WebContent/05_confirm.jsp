...
<%
	...
	String cntStr = request.getParameter("i_cnt");
	String selectedDate = request.getParameter("selectedDate"); 
    String Area = request.getParameter("productArea");
    String Soostr = request.getParameter("productSoo");

  
    int cnt = (cntStr != null) ? Integer.parseInt(cntStr) : 0;
	int productPrice = (productPriceStr != null) ? Integer.parseInt(productPriceStr) : 0;
	int Soo = (Soostr != null) ? Integer.parseInt(Soostr) : 0;
 

	int totprice = productPrice * cnt;  
	...
	<hr>
    	...
    	<div class="displays ">
    	   <div class="mr-4 "><img  class="imgbox" src="./allist/<%=Soo%>.jpg" width="140px" height="80px"></div>
    	     <table border="1" style="border-collapse: collapse; width: 50%; margin-left:20px;">
    	     <tr>
  	  	     	<td>
    	    	 상품 번호: <%=Soo%> <br>
    	    	 상품명: [<%=Area%>] <%=productName %> <br>
    	    	 수량: <%=cnt %> 개 <br>
    	     </td>
    	    
    	     <td>
    	     사용 예정일: <%=selectedDate %> <br>
    	     <input type="hidden" id="setDate" value="<%=selectedDate %>">
    	     상품 가격: <%=df.format(productPrice)  %> 원 <br>			
			 총 금액 : <%=df.format(totprice) %> 원  
    	     </td>
	...
	<div>사용 희망 마일리지: &nbsp;&nbsp;<input class="membox" id="mileage" type="text" placeholder="사용을 원하는 마일리지를 써주세요" value=""></div>
	</div>
	...
	<div method="post" class="displayb Tborer">
		<div  style="marig:20px 0px; widht:600px;" class="m-5">
			<div style="marig:20px 0px; height:30px;" > 결제 아이디 : <%=sid%></div>	
				<div class="tdw" style="display:none;" id="tid"><%=sid%></div>	
			<div style="marig:20px 0px; height:30px;" > 주문 상품 : <%=productName %></div>	
					<div id="productName" style="display:none;"><%=productName%></div>				
		 	<div id="soo" style="display:none;"><%=Soo%></div>	
			<div style="marig:20px 0px; height:30px;" > 주문 수량 :<%=cnt%> 개 </div> 
				<div style="display:none;" id=t_cnt><%=cnt%> </div>
				<div style="marig:20px 0px; height:30px;" > 주문 금액 : <%=df.format(totprice) %> 원 
					<input style="border:none; display:none;" id="amount12" readonly value="<%=totprice%>"></div> 
			<div style="marig:20px 0px; height:30px;" > 사용 마일리지 :  
			 		<input style="border:none;" id="realMileage" readonly value=""></div> 
					<div style="display:none;" id="realm"></div>
			<div style="marig:20px 0px; height:30px;" > 결제 금액 :
			<input style="border:none;" id="realtot" readonly value="<%=df.format(totprice) %> 원">
			</div><div style="display:none;"  id=realprice><%=totprice%> </div>
		</div>
...
function checkM() {
    ...
    if (parseInt(userMileage) > availableMileage) {
        ...
        // 사용자가 입력한 마일리지를 realMileage 엘리먼트에 표시합니다.
         ...
         /* document.getElementById('realMileage').value = userMileage;  */
          document.getElementById('realMileage').value = formatter.format(userMileage) + "M"; 
          document.getElementById('realm').innerText = userMileage; // hiddenmilege 
        ...
        document.getElementById('realtot').value = formatter.format(realtotprice) + "원"; 
       	/* document.getElementById('realtot').value = realtotprice; */
	...
	<script>
	 ...
	 //client-side
	  function requestPay() {
	   ...
	   var mile = document.getElementById("realm").innerText;
